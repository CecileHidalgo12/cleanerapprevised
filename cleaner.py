...
if uploaded_files:
    ...
    # ----------------------------------------------------
    st.subheader("1. Select the Watermarks to Remove")
    st.write("We will extract unique images from your first file. Select **all** the ones you want wiped from ALL files.")
    ...
    # Track multiple selected watermark byte arrays
    selected_watermarks = []
    ...
    if unique_images:
        ...
        for idx, (img_bytes, xref) in enumerate(image_list):
            try:
                with cols[idx % 3]:
                    st.image(img_bytes, caption=f"Option {idx+1} (XRef {xref})", use_container_width=True)
                    # Add a checkbox for each image option
                    if st.checkbox(f"Remove Option {idx+1}", key=f"chk_{xref}"):
    ...
    if st.button("Process All Files"):
        if not selected_watermarks:
            st.error("Please check at least one image box above before running the process.")
        else:
                    ...
                    try:
                        ...
                        for page in doc:
                            for img in page.get_images(full=True):
                                ...
                                # Match against any of the selected watermark signatures
